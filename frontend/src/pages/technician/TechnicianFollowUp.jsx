...
import {
  ...
  selectCurrentTechnicianFollowUpDataSource,
...
function TechnicianFollowUp() {
  ...
  const [followUpDataSource] = useState(() =>
    technician
      ? selectCurrentTechnicianFollowUpDataSource(technician.technician_ID)
      : { records: [], source: 'entity' },
  )
  const followUps = followUpDataSource.records
  ...
  const metrics = useMemo(
    () => ({
      ...
      upcoming: followUps.filter((record) => record.status === 'Upcoming').length,
      inProgress: followUps.filter((record) => record.status === 'In Progress').length,
      completed: followUps.filter((record) => record.status === 'Completed').length,
    }),
    [followUps],
  )
    ...
    <div className="technician-follow-up-page">
      ...
      {followUps.length === 0 ? (
        <section className="follow-up-empty-state follow-up-empty-state-page" role="status">
          ...
          <p>Assigned follow-up bookings will appear here when they are available.</p>
        </section>
        ...
        <div className="follow-up-groups">
          {visibleGroups.map((group) => {
            const groupRecords = filteredFollowUps.filter(
                      ...
                      <article className="follow-up-visit-card" key={`${record.bookingID}:${record.jobID}`}>
                        ...
                        <div className="follow-up-visit-content">
                          ...
                          <dl className="follow-up-visit-details">
                            ...
                            <div className="follow-up-visit-location">
                              <dt>Location</dt>
            ...
            <Modal.Body>
              <div className="follow-up-detail-grid">
                ...
                <section>
                  ...
                  <small>{selectedFollowUp.isFollowupReport === true ? 'Service report flagged follow-up' : 'Follow-up booking'}</small>
                </section>
              </div>
            </Modal.Body>
            <Modal.Footer>
