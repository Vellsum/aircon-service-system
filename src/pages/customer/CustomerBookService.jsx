...
import {
  ...
  SERVICES,
  TIME_SLOTS,
} from "../../data/customer/customerMockData";
import {
  findPromotion,
  findService,
  formatLongDate,
...
const CustomerBookService = () => {
  ...
  // e.g. /customer/book?service=2&unit=9003
  ...
  const [addressId, setAddressId] = useState(
    SAVED_ADDRESSES.find((address) => address.isDefault)?.address_ID ?? null
  );
  ...
  const address = SAVED_ADDRESSES.find((entry) => entry.address_ID === addressId) || null;
  const selectedUnits = MY_UNITS.filter((unit) => unitIds.includes(unit.installed_ID));
  ...
  const applyPromo = () => {
    const promo = findPromotion(promoInput);
    if (promo) {
  ...
  const submitBooking = () => {
    // Demo submit. The real call is POST /api/bookings with
    // { customer_ID, date, time, location } — see bookingController.createBooking.
    const reference = `BK-2026-${Math.floor(1000 + Math.random() * 8999)}`;
    setConfirmed({ reference, total: pricing.total });
        ...
        <div>
                ...
                {MY_UNITS.map((unit) => (
                  ...
                  >
                    ...
                    <p className="cust-choice-desc">
                      {unit.brand} {unit.model} · {unit.capacity}
                    </p>
                    <div className="cust-choice-foot">
                      <span className="cust-duration">
                        <Icon name="history" size={12} />
                        Last: {unit.last_serviced}
                      </span>
                    </div>
                  </button>
                ))}
          ...
          <div className="cust-wizard-nav">
            ...
            {step < STEPS.length ? (
              ...
              <button className="dash-btn dash-btn-primary" onClick={submitBooking}>
